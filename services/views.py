...
from services.common import getencdec, standard_request_handler, typical_json_responder, translate_parameters, parse_json, \
    translate_values, translate_string, proceed_checks, naive_json_responder, clear_last_slash
...
def list_projects_route(pars):  # ++TESTED
    ...
    **Список проектов**
    ...
    Если указан uuid - возвращает информацию по одному проекту, если нет,
    возвращает информацию по открытым проектам удовлетворяющим условию
...
def invite_participant_route(params): # ++TESTED
    ...
    **Пригласить участника**
    ...
    В отладночном режиме возвращает json словарь с ключами:
...
def invitation_response_route(request, invite):
    ...
    r.set_cookie('invite', invite, path = clear_last_slash(settings.STATIC_URL))
    return r
...
def confirmation_response_route(request, confirmation):
    ...
    if st == 200:
        ...
        r.set_cookie('fail', u'Не удалось выполнить подтверждение пользователя', httponly = False, path = clear_last_slash(settings.STATIC_URL))
        transaction.rollback()
...
def check_token_route(params):
    ...
    ** Проверка токена авторизации **
    ...
    путь запроса: **/token/check**

    Параметры запроса:

    - `token`: токен для проверки

    Возвращаемые JSON словарь:

    - `temp`: Bool - если True, то это токен временного пользователя
...
def check_project_participation_route(params):
    ...
    ** Проверка участия пользователя в проекте **
    ...
    - `initiator`: boolean: пользователь - инициатор
...
@transaction.commit_on_success
@standard_request_handler({'token' : ''})
@typical_json_responder(execute_logout, 201)
def logout_route(params):
    """
    ** Logout **
