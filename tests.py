...
class mytest(TestCase):
    ...
    def test_resource_parameters(self, ):
        ...
        # Создаем общий ресурс
        r = self.srequest(c, '/resource/create', {'psid' : psid,
                                                  'name' : 'common',
                                                  'units' : 'kg',
                                                  'use' : 'common',
                                                  'site' : 'external'},
    ...
    def test_activity_resource_costs(self, ):
        ...
        c = httplib.HTTPConnection(host, port)
        enc, dec = getencdec()
        r = self.srequest(c, '/project/create', {'name' : 'someprojname',
                                                 'sharing' : 'open',
                                                 'ruleset' : 'despot',
                                                 'user_name' : 'root'},
                          httplib.CREATED)
        ...
        r = self.srequest(c, '/activity/public', {'psid' : psid,
                                                  'uuid' : auuid},
                          httplib.CREATED)

        r = self.srequest(c, '/activity/participation', {'psid' : psid,
                                                         'action' : 'include',
                                                         'uuid' : auuid},
                          httplib.CREATED)
        ...
        # второй повтащик предлгаает 3 ресурс в неограниченном количестве, но
        # цена 20 денег за еденицу несурса

        # инициаор переводит проект в режим contractor
